...
export function createMcpServer(): McpServer {
  ...
  server.registerTool("dtr_pipeline", { description: "Run a named safe pipeline in an isolated worktree. scope is an optional write allowlist; omit it to allow the target workspace.", inputSchema: { ...dispatchInput, template: z.enum(["debug-review", "plan-challenge-review", "implement-review"]), write: z.boolean().optional(), scope: z.array(z.string().min(1).max(256)).max(32).optional() } }, async (input) => {
    try { const run = await new DtrApplication(configDir, cwd(input.cwd)).pipeline({ ...applicationInput(input), cwd: cwd(input.cwd), files: input.files, template: input.template, ...(input.write !== undefined ? { write: input.write } : {}), ...(input.scope ? { scope: input.scope } : {}) }); return text({ runId: run.record.id, state: run.record.state, stages: run.record.stages }); } catch (reason) { return error(reason); }
  });
