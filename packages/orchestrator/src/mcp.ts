...
export function createMcpServer(): McpServer {
  ...
  server.registerTool("dtr_pipeline", { description: "Run a named safe pipeline in an isolated worktree. scope is an optional write allowlist. Pin implementation and review stages separately; never pin the whole pipeline.", inputSchema: { ...dispatchInput, template: z.enum(["debug-review", "plan-challenge-review", "implement-review"]), write: z.boolean().optional(), scope: z.array(z.string().min(1).max(256)).max(32).optional(), implementationProvider: z.enum(["claude", "codex", "ollama", "openrouter", "featherless", "antigravity", "opencode"]).optional(), reviewProvider: z.enum(["claude", "codex", "ollama", "openrouter", "featherless", "antigravity", "opencode"]).optional() } }, async (input) => {
    try { if (input.allowedProviders?.length) throw new Error("dtr_pipeline does not accept allowedProviders. Use implementationProvider or reviewProvider instead."); const run = await new DtrApplication(configDir, cwd(input.cwd)).pipeline({ ...applicationInput(input), cwd: cwd(input.cwd), files: input.files, template: input.template, ...(input.write !== undefined ? { write: input.write } : {}), ...(input.scope ? { scope: input.scope } : {}), ...(input.implementationProvider ? { implementationProvider: input.implementationProvider } : {}), ...(input.reviewProvider ? { reviewProvider: input.reviewProvider } : {}) }); return text({ runId: run.record.id, state: run.record.state, stages: run.record.stages }); } catch (reason) { return error(reason); }
  });
