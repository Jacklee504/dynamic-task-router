...
export function App({ application, cwd }: { application: DtrApplication; cwd: string }) {
  ...
  useInput((character, key) => {
    if (key.ctrl && character === "c") { const activeId = Object.keys(active)[0]; if (activeId) void application.abort(activeId); else exit(); return; }
    if (key.ctrl && character === "k") { setScreen("dashboard"); return; }
    if (key.ctrl && character === "r") { void refresh().then(() => setScreen("runs")); return; }
    if (key.ctrl && character === "o") { void refresh().then(() => setScreen("models")); return; }
...
function HelpScreen() { return <Panel title="COMMANDS">{commandRegistry.map((command) => <Text key={command.name}>{command.usage} · {command.description}</Text>)}<Text>Keyboard: Ctrl+K dashboard · Ctrl+R runs · Ctrl+O models · Esc back · Up/Down history · Ctrl+C abort active/exit.</Text></Panel>; }
...
function updateActive(previous: Record<string, ActiveRun>, event: import("@dynamic-task-router/orchestrator").DtrEvent): Record<string, ActiveRun> {
  ...
  // Finished runs leave the active map so Ctrl+C and /abort target the next
  // in-flight run and the ACTIVE RUNS panel does not accumulate history.
  if (event.type === "run-completed" || event.type === "run-failed") { const { [event.runId]: _finished, ...rest } = previous; return rest; }
